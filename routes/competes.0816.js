...
const SysConf = require("../models/sysConfSchema")
...
router.post('/userAdd', async(ctx) => {
    ...
    const {username, city, depart, phone, img, divisionId = 0} = util.decrypt(token)
    // 添加人员到系统
    ...
    if(!exitUser) {
        ...
        const result = await User.create({
            userId: counterDoc.sequenceValue, username, city, depart, phone, img, divisionId
        })
        if(result) {
            //将用户写到redis中
            if(result._id) {
        ...
        //更新用户数据
        User.updateOne({_id: exitUser._id}, {city, depart, img, divisionId})
        userInfo._id = exitUser._id
        userInfo.userId = reexitUsersult.userId
        userInfo.username = exitUser.username
        userInfo.phone = exitUser.phone
        userInfo.city = city
...
router.get('/divisionShow', async(ctx) => {
    ...
    //查询当前赛区、当前类型的比赛
    const cIds = await Compete.find({divisionId: competeDivisionId, competeType}, {'cId': 1, _id: 0})
    let cIdArr = []
    if(cIds.length > 0) {
...
router.post('/canInter', async(ctx) => {
    ...
    if(!exitUser) {
        ...
        const cIds = await Compete.find({divisionId, competeType}, {'cId': 1, _id: 0})
        if(cIds.length <= 0) {
            ctx.body = util.success({
                "canInter": true
            })
            ...
            let repeatNum = 0
            if(competeType === 2) {
                repeatNum = finalTimes.content
            }
            if(competeType === 3) {
                repeatNum = weekTimes.content
            }
            if(scoreCount <= repeatNum) {
                ctx.body = util.success({
                    "canInter": true
                })
            } else {
